...
export const DashboardPage: React.FC = () => {
  ...
  const [selectedKpiForChart, setSelectedKpiForChart] = useState<KPISummaryCard | null>(null);

  // Progressive Disclosure Drawer State
  const [inspectedDetection, setInspectedDetection] = useState<DetectionEvent | null>(null);
  const [inspectedRecommendation, setInspectedRecommendation] = useState<Recommendation | null>(null);
  ...
  const fetchAllDashboardData = async () => {
    try {
      setLoading(true);
      setErrorMsg(null);
      const [kpiData, detData, recData, invData, infoData, prevData] = await Promise.all([
      ...
      setErrorMsg('Could not connect to the continuous intelligence backend. Please verify your connection.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLoadSample = async () => {
    try {
      ...
      toast.info('Ingesting realistic 30-day company dataset...', 'Data Pipeline');
      await dataApi.loadSampleDataset();
      toast.success('Sample dataset loaded! Running measurement & anomaly detection.', 'Intelligence Engine');
  ...
  const handleAcknowledgeDetection = async (detId: number) => {
    try {
      await fetch(`/api/v1/detections/${detId}/acknowledge`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('datalyze_token')}`,
        },
      });
      setDetections((prev) =>
        prev.map((d) => (d.id === detId ? { ...d, status: 'acknowledged' } : d))
      );
      toast.success('Anomaly divergence acknowledged and logged to audit trail.', 'Triage Complete');
      if (inspectedDetection?.id === detId) {
        setInspectedDetection(null);
      }
    } catch (err) {
      console.error('Failed to acknowledge detection', err);
      toast.error('Could not update anomaly status. Please try again.', 'Action Failed');
    }
  };
  ...
  const handleStatusChange = async (
    ...
    try {
      const res = await fetch(`/api/v1/recommendations/${recId}/status?status=${newStatus}`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('datalyze_token')}`,
        },
      ...
      if (res.ok) {
        ...
        toast.success(`Action initiative updated to "${newStatus.replace('_', ' ')}".`, 'Operational Prescriptions');
        if (inspectedRecommendation?.id === recId) {
          setInspectedRecommendation((prev) => (prev ? { ...prev, status: newStatus } : null));
        }
      }
    } catch (err) {
      console.error('Failed to update recommendation status', err);
      toast.error('Failed to update recommendation status.', 'Action Failed');
    }
  };

  const hasData = kpis.some((k) => k.current_value !== null && k.current_value !== undefined);
  const healthyCount = kpis.filter((k) => k.status === 'healthy' && k.current_value !== null).length;
  const warningCount = kpis.filter((k) => k.status === 'warning' && k.current_value !== null).length;
  const criticalCount = kpis.filter((k) => k.status === 'critical' && k.current_value !== null).length;
  const activeDetections = detections.filter((d) => d.status === 'active');
  const criticalDetections = activeDetections.filter((d) => d.severity === 'critical');
  const openRecommendations = recommendations.filter((r) => r.status === 'open');
  const criticalInventoryCount = (inventory?.items || []).filter((i) => i.stockout_risk === 'critical').length;

  const totalNeedsAttention =
    criticalDetections.length + warningCount + openRecommendations.length + criticalInventoryCount;

  const activeDetectionsCount = (detections || []).filter((d) => (d?.status || '').toLowerCase() === 'active').length;
  const dynamicEyebrow =
    activeDetectionsCount > 0
      ? `${activeDetectionsCount} Active Alert${activeDetectionsCount === 1 ? '' : 's'} • ${kpis.length} Monitored Numbers`
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-fade-in">
      ...
      <PageHeader
        ...
        title={`Welcome back, ${user?.full_name?.split(' ')[0] || 'Leader'}`}
        ...
        actions={
          <>
            ...
            <Button
              ...
              leftIcon={<Database className="w-3.5 h-3.5 text-[#6B4226] dark:text-[#8C5E3C]" />}
            >
              Upload Data
            </Button>
            <Button
      ...
      {/* State Container: Handles Loading, Error, Empty, and Populated */}
      <StateView
        isLoading={loading}
        isError={!!errorMsg}
      ...
      >
        {/* FIRST-RUN ONBOARDING SCREEN (When 0 records exist) */}
        {!hasData ? (
          <div className="bg-white dark:bg-[#15171C] border border-neutral-200 dark:border-neutral-800 rounded-2xl p-6 sm:p-10 text-center space-y-6 shadow-xs relative overflow-hidden">
            ...
            <div className="max-w-xl mx-auto space-y-2">
              ...
              <p className="text-xs sm:text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed font-normal">
                Upload your company spreadsheet or operational data (Excel, CSV, Word table, or PDF) to automatically extract your metrics, anomaly alerts, 7-day predictions, and recommendations.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-left max-w-3xl mx-auto pt-2">
              <div className="p-3.5 rounded-xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-200/80 dark:border-neutral-800 space-y-1">
                ...
                <p className="text-[11px] text-neutral-500">
                  Drop your Excel, CSV, Word, or PDF file into the Data Pipeline.
                </p>
              </div>
              <div className="p-3.5 rounded-xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-200/80 dark:border-neutral-800 space-y-1">
                ...
                <p className="text-[11px] text-neutral-500">
                  Columns, trends, and unusual changes are detected automatically from your file.
                </p>
              </div>
              <div className="p-3.5 rounded-xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-200/80 dark:border-neutral-800 space-y-1">
                ...
                <p className="text-[11px] text-neutral-500">
                  Receive statistical 7-day predictions and smart operational recommendations.
                </p>
              </div>
            </div>
          ...
          <div className="space-y-6">
            {/* ACTIVE INGESTED DATASET SOURCE BANNER */}
                  ...
                  <div className="overflow-hidden">
                    <div className="flex items-center space-x-2 flex-wrap">
                      <span className="font-bold text-sm text-neutral-900 dark:text-neutral-100 font-mono truncate">
                        Connected Data File: {datasetInfo.filename || 'Uploaded File'}
                      </span>
                    ...
                    <p className="text-xs text-neutral-500 dark:text-neutral-400 font-mono mt-0.5">
                      {(datasetInfo.total_rows || datasetInfo.row_count || 0).toLocaleString()} rows processed •{' '}
                      {datasetInfo.total_columns || datasetInfo.col_count || 0} columns • Currency: {company?.currency || 'INR'} • Time Window: {timeRange}
                    </p>
                  </div>
                </div>
            ...
            {/* 4 HIGH-CLARITY EXECUTIVE HEALTH & ACTION SUMMARY BOXES */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {/* Box 1: Total Key Metrics */}
              <Card
              ...
              >
                ...
                <div className="mt-3">
                  <div className="flex items-baseline space-x-2">
                    ...
                    <span className="text-xs font-semibold text-neutral-500">Tracked Metrics</span>
                  </div>
                  <p className="text-[11px] text-neutral-500 dark:text-neutral-400 mt-1 leading-relaxed">
                    All number columns from your uploaded file are tracked live.
                  </p>
                </div>
                ...
                <div className="mt-3 pt-2.5 border-t border-neutral-100 dark:border-neutral-800/80 flex items-center justify-between text-[11px]">
                  ...
                  <span className="text-[#6B4226] dark:text-[#8C5E3C] font-semibold">View All →</span>
                </div>
              </Card>

              {/* Box 2: Healthy Metrics */}
              <Card
                ...
                <div className="mt-3">
                  <div className="flex items-baseline space-x-2">
                    ...
                    <span className="text-xs font-semibold text-emerald-700 dark:text-emerald-300">
                      ({kpis.length > 0 ? ((healthyCount / kpis.length) * 100).toFixed(0) : 100}% Healthy)
                    </span>
                  </div>
                  <p className="text-[11px] text-neutral-500 dark:text-neutral-400 mt-1 leading-relaxed">
                    Numbers are tracking smoothly within normal daily targets.
                  </p>
                </div>

                <div className="mt-3 pt-2.5 border-t border-neutral-100 dark:border-neutral-800/80 flex items-center justify-between text-[11px]">
                  <span className="text-neutral-400 font-mono">
                    {statusFilter === 'HEALTHY' ? '● Filter Active' : 'Click to filter'}
                  </span>
                  <span className="text-emerald-600 dark:text-emerald-400 font-semibold">Filter Healthy →</span>
                </div>
              </Card>

              {/* Box 3: Warning Changes */}
              <Card
              ...
              >
                ...
                <div className="mt-3">
                  ...
                  <p className="text-[11px] text-neutral-500 dark:text-neutral-400 mt-1 leading-relaxed">
                    Metrics showing noticeable variations that may need your attention.
                  </p>
                </div>

                <div className="mt-3 pt-2.5 border-t border-neutral-100 dark:border-neutral-800/80 flex items-center justify-between text-[11px]">
                  <span className="text-neutral-400 font-mono">
                    {statusFilter === 'WARNING' ? '● Filter Active' : 'Click to filter'}
                  </span>
                  <span className="text-amber-600 dark:text-amber-400 font-semibold">Filter Warnings →</span>
                </div>
              </Card>

              {/* Box 4: Critical Alerts */}
              <Card
              ...
              >
                ...
                <div className="mt-3">
                  ...
                  <p className="text-[11px] text-neutral-500 dark:text-neutral-400 mt-1 leading-relaxed">
                    Significant drops or unusual changes that require immediate action.
                  </p>
                </div>

                <div className="mt-3 pt-2.5 border-t border-neutral-100 dark:border-neutral-800/80 flex items-center justify-between text-[11px]">
                  <span className="text-neutral-400 font-mono">
                    {statusFilter === 'CRITICAL' ? '● Filter Active' : 'Click to filter'}
                  </span>
                  <span className="text-rose-600 dark:text-rose-400 font-semibold">Filter Critical →</span>
                </div>
              </Card>
            </div>

            {/* FILTER STATUS PILL (Shows when a filter is applied) */}
            {statusFilter !== 'ALL' && (
              <div className="bg-[#FAF8F5] dark:bg-[#181A20] border border-neutral-300 dark:border-neutral-700 rounded-xl p-3 flex items-center justify-between shadow-2xs">
                <div className="flex items-center space-x-2 text-xs font-mono">
                  ...
                  <span className="text-neutral-500">
                    ({kpis.filter((k) => k.status.toUpperCase() === statusFilter).length} of {kpis.length} shown)
                  </span>
                </div>
                <button
                  onClick={() => setStatusFilter('ALL')}
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <h3 className="text-xs font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider font-mono">
                    Tracked Metric Baselines & Sparklines {statusFilter !== 'ALL' ? `(${statusFilter})` : ''}
                  </h3>
                  {statusFilter !== 'ALL' && (
                    <Badge variant={statusFilter === 'CRITICAL' ? 'critical' : statusFilter === 'WARNING' ? 'warning' : 'healthy'} size="xs">
                      {kpis.filter((k) => k.status.toUpperCase() === statusFilter).length} OF {kpis.length} SHOWN
                    </Badge>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {kpis
                  .filter((k) => statusFilter === 'ALL' || k.status.toUpperCase() === statusFilter)
                  .map((kpi) => (
                    <KpiCard key={kpi.id || kpi.key} kpi={kpi} />
                  ))}
              </div>
            </div>

            {/* Active Detections & Prescriptions Split Grid with Progressive Disclosure */}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <h3 className="text-sm font-bold text-neutral-900 dark:text-neutral-100 tracking-tight">
                      Active Baseline Divergences ({activeDetections.length})
                    </h3>
                  </div>
                  <button
                ...
                {activeDetections.length === 0 ? (
                  <div className="bg-white dark:bg-[#15171C] border border-neutral-200 dark:border-neutral-800 rounded-2xl p-6 text-center space-y-2 border-l-4 border-l-emerald-500">
                    ...
                    <p className="text-xs text-neutral-500">
                      Statistical z-score engine detected no significant divergences in the last 7 days.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {activeDetections.slice(0, 3).map((det) => (
                      ...
                      >
                        <DetectionCard
                          detection={det}
                          onAcknowledge={handleAcknowledgeDetection}
                        />
                      </div>
                    ))}
                  </div>
              ...
              <div className="space-y-3.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <h3 className="text-sm font-bold text-neutral-900 dark:text-neutral-100 tracking-tight">
                      Prescriptive Recommendations ({openRecommendations.length})
                    </h3>
                  </div>
                  <button
                ...
                {openRecommendations.length === 0 ? (
                  <div className="bg-white dark:bg-[#15171C] border border-neutral-200 dark:border-neutral-800 rounded-2xl p-6 text-center space-y-2 border-l-4 border-l-emerald-500">
                    ...
                    <p className="text-xs text-neutral-500">
                      All recommended operational directives have been reviewed or resolved.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {openRecommendations.slice(0, 3).map((rec) => (
                      <div
                        key={rec.id}
                        onClick={() => setInspectedRecommendation(rec)}
                        className="cursor-pointer transition-transform hover:scale-[1.01]"
                      >
                        <RecommendationCard
                          recommendation={rec}
            ...
            {/* 5. UPLOADED DATASET LIVE RECORDS MATRIX */}
            {datasetPreview && datasetPreview.records && datasetPreview.records.length > 0 && (
              <Card className="p-5 sm:p-6 space-y-4">
                <CardHeader className="px-0 pt-0 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center space-x-2.5">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        <CardTitle>Uploaded File Records & Live Matrix</CardTitle>
                        <Badge variant="healthy" size="xs">
                          {datasetPreview.total_rows || datasetPreview.records.length} TOTAL ROWS
                        </Badge>
                      </div>
                      <p className="text-xs text-neutral-500 font-normal mt-0.5">
                        Direct cell inspection from {datasetInfo?.filename || 'uploaded document'} • Showing first 10 rows
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Button
                      ...
                      size="xs"
                      onClick={() => navigate('/data')}
                      leftIcon={<Eye className="w-3 h-3 text-[#6B4226] dark:text-[#8C5E3C]" />}
                    >
                      Open & Edit File →
                    </Button>
                    <Button
                      ...
                      size="xs"
                      onClick={() => navigate('/data')}
                      leftIcon={<Database className="w-3 h-3 text-[#6B4226] dark:text-[#8C5E3C]" />}
                    >
                      Query Sandbox →
                    </Button>
                  </div>
                ...
                <div className="overflow-x-auto rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-[#15171C]">
                  <table className="w-full min-w-max text-xs text-left">
                    <thead className="bg-neutral-50 dark:bg-neutral-900/80 text-neutral-600 dark:text-neutral-300 font-semibold font-mono border-b border-neutral-200 dark:border-neutral-800">
                      <tr>
                        <th className="px-3.5 py-2.5 w-12 text-neutral-400">#</th>
                        {(datasetPreview.columns || []).map((col: string) => (
                          <th key={col} className="px-3.5 py-2.5 whitespace-nowrap min-w-[120px]">
                      ...
                      {datasetPreview.records
                        ...
                        .map((row: any, idx: number) => (
                          ...
                          >
                            ...
                            {(datasetPreview.columns || []).map((col: string) => (
                              <td key={col} className="px-3.5 py-2 text-neutral-800 dark:text-neutral-200 min-w-[120px]">
      ...
      {/* Progressive Disclosure Drawer: Anomaly Detail */}
      <Drawer
        isOpen={!!inspectedDetection}
        onClose={() => setInspectedDetection(null)}
        title={
          inspectedDetection
            ? `${inspectedDetection.kpi_name || 'Metric'} Baseline Divergence`
            : 'Anomaly Divergence Details'
        }
        subtitle={`Detected on ${inspectedDetection?.detected_at ? inspectedDetection.detected_at.split('T')[0] : 'recent timeline'}`}
            ...
            <div className="flex items-center space-x-2">
              ...
              {inspectedDetection.status === 'active' && (
                <Button
                  variant="primary"
                  size="sm"
                  onClick={() => handleAcknowledgeDetection(inspectedDetection.id)}
        ...
        {inspectedDetection && (
          <div className="space-y-5">
            <div className="flex items-center justify-between p-3.5 rounded-xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800">
              <span className="text-xs text-neutral-500 font-mono">Severity Rating:</span>
              <Badge
                variant={
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-neutral-800 dark:text-neutral-200 leading-relaxed">
                Reading of {inspectedDetection.current_value} diverged by{' '}
                {inspectedDetection.percentage_change}% from baseline ({inspectedDetection.baseline_value}).
              </p>
            </div>

            {inspectedDetection.root_causes && inspectedDetection.root_causes.length > 0 && (
                    ...
                    >
                      <div>
                        ...
                        <span className="text-[10px] text-neutral-500 font-mono">
                          {rc.dimension_name}: {rc.dimension_value}
                        </span>
                      </div>
                      <span className="text-amber-600 dark:text-amber-400 font-bold shrink-0">
                        {rc.contribution_percentage.toFixed(0)}% contribution
                      </span>
                    </div>
                  ))}
                </div>
      ...
      {/* Progressive Disclosure Drawer: Recommendation Detail */}
      <Drawer
        isOpen={!!inspectedRecommendation}
        onClose={() => setInspectedRecommendation(null)}
        title={inspectedRecommendation?.title || 'Prescriptive Initiative Details'}
        subtitle={`Priority: ${inspectedRecommendation?.priority.toUpperCase()} • Impact: ${inspectedRecommendation?.impact_level?.toUpperCase() || 'HIGH'}`}
          ...
          inspectedRecommendation && (
            <div className="flex items-center space-x-2">
              <Button
                variant="secondary"
                size="sm"
                onClick={() => setInspectedRecommendation(null)}
              >
                Close
              </Button>
              {inspectedRecommendation.status !== 'completed' && (
                <Button
                  variant="primary"
                  size="sm"
                  onClick={() => handleStatusChange(inspectedRecommendation.id, 'completed')}
                  leftIcon={<Check className="w-3.5 h-3.5" />}
                >
                  Mark as Resolved
                </Button>
      ...
      >
        {inspectedRecommendation && (
          <div className="space-y-5">
            <div className="flex items-center justify-between p-3.5 rounded-xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800">
              <span className="text-xs text-neutral-500 font-mono">Current Status:</span>
              <Badge
                variant={
                  inspectedRecommendation.status === 'completed'
                    ? 'healthy'
                    : inspectedRecommendation.status === 'in_progress'
                    ? 'info'
                    : 'warning'
                }
              >
            ...
            <div className="space-y-1.5">
              ...
              {inspectedRecommendation.rationale && (
                <p className="text-xs text-neutral-500 pt-1 leading-relaxed">
                  Rationale: {inspectedRecommendation.rationale}
                </p>
              )}
            </div>
