...
export const EmployeeForgotPasswordPage: React.FC = () => {
  ...
  const handleRequestSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err.response?.data?.detail || 'Account not found. Please verify your employee email or phone.';
      setError(msg);
    ...
    <div className="min-h-screen bg-[#FAF8F5] dark:bg-[#0C0D10] flex flex-col justify-center py-10 px-4 sm:px-6 lg:px-8 relative overflow-hidden font-sans text-neutral-900 dark:text-neutral-100">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center relative z-10 space-y-2">
        ...
        <h2 className="text-2xl font-extrabold tracking-tight text-neutral-900 dark:text-neutral-100">
          {step === 4 ? 'Password Changed!' : 'Reset Employee Password'}
        </h2>
        <p className="text-xs sm:text-sm text-neutral-500 dark:text-neutral-400 max-w-sm mx-auto font-normal">
          {step === 1 && 'Enter your registered employee email or phone to receive a verification code'}
          {step === 2 && `Enter the 6-digit verification code sent to ${targetMasked}`}
          {step === 3 && 'Create a new secure password for your employee account'}
          {step === 4 && 'Your employee password has been updated in the database'}
        </p>
      </div>
        ...
        <Card className="py-6 px-5 sm:px-8 shadow-md space-y-5 border-t-4 border-t-blue-600">
          ...
          {step === 1 && (
            <form onSubmit={handleRequestSubmit} className="space-y-4">
              <FormField label="Employee Email or Phone" required error={fieldErrors.identifier}>
                <Input
                  type="text"
                  required
            ...
            <form onSubmit={handleVerifySubmit} className="space-y-4">
              {codePreview && (
                <div className="p-3 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900 text-xs text-blue-900 dark:text-blue-200 space-y-1">
                  ...
                  <p className="font-mono text-base font-extrabold tracking-widest text-blue-700 dark:text-blue-300">
                    {codePreview}
                  </p>
          ...
          {step === 4 && (
            <div className="text-center py-4 space-y-4 animate-fade-in">
              ...
              <p className="text-xs text-neutral-600 dark:text-neutral-400 leading-relaxed">
                Your employee password has been updated in the database. You may now sign in using your new credentials.
              </p>
              <Button
                variant="primary"
                size="md"
          ...
          {step !== 4 && (
            <div className="pt-3 border-t border-neutral-100 dark:border-neutral-800 text-center text-xs">
              <Link to="/login/employee" className="font-semibold text-blue-600 dark:text-blue-400 hover:underline">
                ← Return to Employee Sign In
              </Link>
            </div>
          )}
